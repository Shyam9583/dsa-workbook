...
public class SmallestWindowContainingOtherStringCharacters {
    ...
    // similar to single java.dsaworkbook.string case but, distinct is found from second java.dsaworkbook.string
    private static String smallestWindow(String first, String second) {
        ...
        while (end < M) {
            ...
            // current count is increased if first count == 1 and character is present in second java.dsaworkbook.string
            if (firstCount[first.charAt(end)] == 1 && secondVisited[first.charAt(end)]) {
                currentCount++;
            }
            if (currentCount == distinctCount) {
                // reduce window size if start character is redundant or character not present in second java.dsaworkbook.string (so we skip it)
                while (firstCount[first.charAt(start)] > 1 || !secondVisited[first.charAt(start)]) {
                    ...
                    // redundant character which is also present in second java.dsaworkbook.string
