...
public class PrintSortedMatrix {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int test = 0; test < testCases; test++) {
            ...
            int[] inputArray = Arrays.stream(bufferedReader.readLine().trim().split(" ")).limit(size * size).mapToInt(Integer::parseInt).toArray();
...
////            int[] inputArray = Arrays.stream(bufferedReader.readLine().trim().split(" ")).mapToInt(Integer::parseInt).toArray();
//////            for(int i = 0; i < size; i++) {
//////                for(int j = 0; j < size;j++) {
//////                    matrix[i][j] = inputArray[i * size + j];
//////                }
//////            }
////            Arrays.sort(inputArray);
////            for(int el: inputArray) {
