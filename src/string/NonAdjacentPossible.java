...
public class NonAdjacentPossible {
    ...
    public static void main(String[] args) throws IOException {
        ...
        PriorityQueue<Pair> heap = new PriorityQueue<>((o1, o2) -> {
            if (o1.count == o2.count) {
                return o1.character < o2.character ? -1 : 1;
            } else return o1.count > o2.count ? -1 : 1;
        });
    ...
    // use special priority queue as heap
    private static int possible(String s, int N, PriorityQueue<Pair> heap) {
        ...
        for (int i = 0; i < 26; i++) {
            if (count[i] > 0) {
                // if more than half of the string has same character, it's bound be adjacent
                if (count[i] > (N + 1) / 2)
                    return 0;
        ...
        // final string constructed has same length as original string
        return size == N ? 1 : 0;
    }
